...
class FaceEnrollClient extends EnrollClient<IFaceService> {
    ...
    private final int[] mEnrollIgnoreList =
            getContext().getResources().getIntArray(
                    R.array.config_face_acquire_enroll_ignorelist);
    ...
    protected boolean hasReachedEnrollmentLimit() {
        if (mBiometricUtils.getBiometricsForUser(getContext(), getTargetUserId()).size()
    ...
    protected void startHalOperation() {
        ...
        try {
            getFreshDaemon().enroll(ArrayUtils.toByteArray(token), mTimeoutSec,
                    ArrayUtils.toIntArray(disabledFeatures));
