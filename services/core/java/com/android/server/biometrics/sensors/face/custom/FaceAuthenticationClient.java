...
class FaceAuthenticationClient extends AuthenticationClient<IFaceService> {
    ...
    FaceAuthenticationClient(Context context,
        ...
        mCustomHaptics =
                Settings.Global.getInt(contentResolver, "face_custom_success_error", 0) == 1;
    }
    ...
    public void onAuthenticated(BiometricAuthenticator.Identifier identifier,
        ...
        mUsageStats.addEvent(new UsageStats.AuthenticationEvent(getStartTimeMs(),
                System.currentTimeMillis() - getStartTimeMs(), authenticated, 0, 0,
                getTargetUserId()));
    ...
    public void onError(int error, int vendorCode) {
        mUsageStats.addEvent(new UsageStats.AuthenticationEvent(getStartTimeMs(),
                System.currentTimeMillis() - getStartTimeMs(), false, error, vendorCode,
                getTargetUserId()));
        if (error == 16) {
