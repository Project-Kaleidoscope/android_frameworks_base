...
public class BiometricTestSessionImpl extends ITestSession.Stub {
    ...
    public BiometricTestSessionImpl(Context context, int sensorId, ITestSessionCallback callback,
                                    CustomFaceProvider customFaceProvider,
                                    CustomFaceProvider.HalResultController halResultController) {
        mContext = context;
        mSensorId = sensorId;
        mCallback = callback;
    ...
    public void startEnroll(int userId) {
        Utils.checkPermission(mContext, "android.permission.TEST_BIOMETRIC");
        mCustomFaceProvider.scheduleEnroll(mSensorId, new Binder(), new byte[69],
                userId, mReceiver, mContext.getOpPackageName(), new int[0], null, false);
    }
    ...
    public void acceptAuthentication(int userId) {
        ...
        if (faces.isEmpty()) {
            ...
            mHalResultController.onAuthenticated(faces.get(0).getBiometricId(),
                    userId, new byte[]{0});
        }
    }
    ...
    public void cleanupInternalState(int userId) {
        Utils.checkPermission(mContext, "android.permission.TEST_BIOMETRIC");
        mCustomFaceProvider.scheduleInternalCleanup(mSensorId, userId,
