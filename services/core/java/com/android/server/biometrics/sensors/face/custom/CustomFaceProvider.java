...
public class CustomFaceProvider implements ServiceProvider {
    ...
    CustomFaceProvider(Context context, FaceSensorPropertiesInternal sensorProps,
                       LockoutResetDispatcher lockoutResetDispatcher,
                       BiometricScheduler scheduler) {
        ...
        HalResultController halResultController = new HalResultController(sensorProps.sensorId,
                context, handler, scheduler, lockoutHalImpl, lockoutResetDispatcher);
    ...
    public CustomFaceProvider(Context context, FaceSensorPropertiesInternal sensorProps,
                              LockoutResetDispatcher lockoutResetDispatcher) {
    ...
    private int decrementChallengeCount() {
        mGeneratedChallengeCount.removeIf(
            aLong -> sSystemClock.millis() - aLong > GENERATE_CHALLENGE_COUNTER_TTL_MILLIS);
    ...
    @Override
    public void scheduleGenerateChallenge(int sensorId, int userId, IBinder token,
        ...
        mHandler.post(() -> {
            if (getDaemon() == null) {
                ...
                final FaceGenerateChallengeClient client =
                        new FaceGenerateChallengeClient(mContext, mLazyDaemon, token,
                ...
                mScheduler.scheduleClientMonitor(client, new BaseClientMonitor.Callback() {
                    ...
                    public void onClientStarted(BaseClientMonitor clientMonitor) {
                        if (client != clientMonitor) {
                            Slog.e(TAG, "scheduleGenerateChallenge onClientStarted, mismatched " +
                                    "client. Expecting: " + client + ", received: "
                                    + clientMonitor);
                        }
                    }
                });
    ...
    @Override
    public void scheduleRevokeChallenge(int sensorId, int userId, IBinder token,
        ...
        mHandler.post(() -> {
            ...
            mScheduler.scheduleClientMonitor(client, new BaseClientMonitor.Callback() {
                ...
                public void onClientFinished(BaseClientMonitor clientMonitor, boolean success) {
                    if (client != clientMonitor) {
                        Slog.e(TAG,
                                "scheduleRevokeChallenge, mismatched client.Expecting: " + client
    ...
    @Override
    public long scheduleFaceDetect(int sensorId, IBinder token, int userId,
                                   ClientMonitorCallbackConverter callback, String opPackageName,
                                   int statsClient) {
        throw new IllegalStateException("Face detect not supported by IBiometricsFace@1.0. Did " +
                "youforget to check the supportsFaceDetection flag?");
    }
    ...
    @Override
    public void scheduleAuthenticate(int sensorId, IBinder token, long operationId, int userId,
                                     int cookie, ClientMonitorCallbackConverter receiver,
                                     String opPackageName, long requestId, boolean restricted,
        ...
        mHandler.post(() -> {
            if (getDaemon() == null) {
                ...
                scheduleUpdateActiveUserWithoutHandler(userId);
                mScheduler.scheduleClientMonitor(new FaceAuthenticationClient(mContext,
                        mLazyDaemon, token, receiver, userId, operationId, restricted,
                        opPackageName, cookie, false, mSensorId,
                        Utils.isStrongBiometric(mSensorId), statsClient, mLockoutTracker,
    ...
    @Override
    public void scheduleRemove(int sensorId, IBinder token, int faceId, int userId,
                               IFaceServiceReceiver receiver, String opPackageName) {
        mHandler.post(() -> {
            if (getDaemon() == null) {
                bindFaceAuthService(mCurrentUserId);
                ...
                scheduleUpdateActiveUserWithoutHandler(userId);
                mScheduler.scheduleClientMonitor(new FaceRemovalClient(mContext, mLazyDaemon,
                        token, new ClientMonitorCallbackConverter(receiver), faceId, userId,
    ...
    @Override
    public void scheduleRemoveAll(int sensorId, IBinder token, int userId,
                                  IFaceServiceReceiver receiver, String opPackageName) {
        mHandler.post(() -> {
            if (getDaemon() == null) {
                bindFaceAuthService(mCurrentUserId);
                ...
                scheduleUpdateActiveUserWithoutHandler(userId);
                mScheduler.scheduleClientMonitor(new FaceRemovalClient(mContext, mLazyDaemon,
                        token, new ClientMonitorCallbackConverter(receiver), 0, userId,
                        opPackageName, FaceUtils.getLegacyInstance(mSensorId), mSensorId,
                        mAuthenticatorIds));
            }
        });
    ...
    public void scheduleResetLockout(int sensorId, int userId, byte[] hardwareAuthToken) {
        mHandler.post(() -> {
            if (getDaemon() == null) {
                ...
                scheduleUpdateActiveUserWithoutHandler(userId);
                mScheduler.scheduleClientMonitor(new FaceResetLockoutClient(mContext, mLazyDaemon
                        , userId, mContext.getOpPackageName(), mSensorId, hardwareAuthToken));
            }
        });
    }

    @Override
    public void scheduleSetFeature(int sensorId, IBinder token, int userId, int feature,
                                   boolean enabled, byte[] hardwareAuthToken,
                                   IFaceServiceReceiver receiver, String opPackageName) {
        mHandler.post(() -> {
            if (getDaemon() == null) {
                bindFaceAuthService(mCurrentUserId);
            ...
            scheduleUpdateActiveUserWithoutHandler(userId);
            mScheduler.scheduleClientMonitor(new FaceSetFeatureClient(mContext, mLazyDaemon,
                    token, new ClientMonitorCallbackConverter(receiver), userId, opPackageName,
                    mSensorId, feature, enabled, hardwareAuthToken, faces.get(0).getBiometricId()));
        });
    }

    @Override
    public void scheduleGetFeature(int sensorId, IBinder token, int userId, int feature,
                                   ClientMonitorCallbackConverter listener, String opPackageName) {
        mHandler.post(() -> {
            if (getDaemon() == null) {
                bindFaceAuthService(mCurrentUserId);
                if (listener != null) {
                ...
                final FaceGetFeatureClient client = new FaceGetFeatureClient(mContext,
                        mLazyDaemon, token, listener, userId, opPackageName, mSensorId, feature,
                        faces.get(0).getBiometricId());
                mScheduler.scheduleClientMonitor(client, new BaseClientMonitor.Callback() {
                    @Override
                    public void onClientFinished(BaseClientMonitor clientMonitor, boolean success) {
                        if (success && feature == 1) {
                            final int settingsValue = client.getValue() ? 1 : 0;
                            Slog.d(TAG, "Updating attention value for user: " + userId + " to " +
                                    "value: " + settingsValue);
                            Settings.Secure.putIntForUser(mContext.getContentResolver(),
                                    "face_unlock_attention_required", settingsValue, userId);
                        }
                    }
                });
            }
    ...
    void scheduleInternalCleanup(int userId, BaseClientMonitor.Callback callback) {
        mHandler.post(() -> {
            ...
            mScheduler.scheduleClientMonitor(new FaceInternalCleanupClient(mContext, mLazyDaemon,
                    userId, opPackageName, mSensorId, enrolledList,
                    FaceUtils.getLegacyInstance(mSensorId), mAuthenticatorIds), callback);
        });
    }
    ...
    @Override
    public void scheduleInvalidateAuthenticatorId(int i, int i1,
    ...
    void scheduleUpdateActiveUserWithoutHandler(final int targetUserId) {
        mScheduler.scheduleClientMonitor(new FaceUpdateActiveUserClient(mContext, mLazyDaemon,
                        targetUserId, mContext.getOpPackageName(), mSensorId, mCurrentUserId,
                        !getEnrolledFaces(mSensorId, targetUserId).isEmpty(), mAuthenticatorIds),
            ...
            if (userId != -10000 && getFaceService(userId) == null) {
                try {
                    ...
                    boolean result = mContext.bindServiceAsUser(intent,
                            new FaceServiceConnection(userId), 1, UserHandle.of(userId));
                    if (result) {
    ...
    @Override
    public ITestSession createTestSession(int sensorId, ITestSessionCallback callback,
    ...
    public static class HalResultController
            extends com.android.internal.util.custom.faceunlock.IFaceServiceReceiver.Stub {
        private final Context mContext;
        private final Handler mHandler;
        ...
        HalResultController(int sensorId, Context context, Handler handler,
                            BiometricScheduler scheduler, LockoutHalImpl lockoutTracker,
                            LockoutResetDispatcher lockoutResetDispatcher) {
            mSensorId = sensorId;
            mContext = context;
            mHandler = handler;
        ...
        public void onEnrollResult(int faceId, int userId, int remaining) {
            mHandler.post(() -> {
                Face face =
                        new Face(FaceUtils.getLegacyInstance(mSensorId).getUniqueName(mContext,
                                userId), faceId, DEVICE_ID);
                BaseClientMonitor client = mScheduler.getCurrentClient();
                if (!(client instanceof FaceEnrollClient)) {
                    Slog.e(TAG,
                            "onEnrollResult for non-enroll client: " + Utils.getClientName(client));
                    return;
                }
        ...
        public void onAuthenticated(int faceId, int userId, byte[] token) {
            mHandler.post(() -> {
                ...
                if (!(client instanceof AuthenticationConsumer)) {
                    Slog.e(TAG,
                            "onAuthenticated for non-authentication consumer: "
                                    + Utils.getClientName(client));
                    return;
                }
                ((AuthenticationConsumer) client).onAuthenticated(new Face("", faceId, DEVICE_ID)
                        , faceId != 0, ArrayUtils.toByteArrayList(token));
            });
        }
        ...
        public void onAcquired(int userId, int acquiredInfo, int vendorCode) {
            mHandler.post(() -> {
                ...
                if (!(client instanceof AcquisitionClient)) {
                    Slog.e(TAG,
                            "onAcquired for non-acquire client: " + Utils.getClientName(client));
                    return;
                }
        ...
        public void onRemoved(int[] removed, int userId) {
            mHandler.post(() -> {
                ...
                if (!(client instanceof RemovalConsumer)) {
                    Slog.e(TAG,
                            "onRemoved for non-removal consumer: " + Utils.getClientName(client));
                    return;
                }
        ...
        public void onEnumerate(int[] faceIds, int userId) {
            mHandler.post(() -> {
                ...
                if (!(client instanceof EnumerateConsumer)) {
                    Slog.e(TAG,
                            "onEnumerate for non-enumerate consumer: "
                                    + Utils.getClientName(client));
                    return;
                }
                ...
                if (faceIds.length > 0) {
                    for (int i = 0; i < faceIds.length; i++) {
                        enumerateConsumer.onEnumerationResult(new Face("", faceIds[i], DEVICE_ID)
                                , (faceIds.length - i) - 1);
                    }
                    return;
                }
                ...
                synchronized (mFaceServices) {
                    try {
                        ...
                        mHandler.post(() -> {
                            ...
                            scheduleGetFeature(mSensorId, new Binder(), mUserId, 1,
