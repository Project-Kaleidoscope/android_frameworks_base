...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private void initDialog() {
        ...
        if (!mShowActiveStreamOnly) {
            ...
            lp.gravity |= mLeftVolumeRocker ? Gravity.LEFT : Gravity.RIGHT;
        }
        ...
        mDialogView.setLayoutDirection(
                mLeftVolumeRocker ? LAYOUT_DIRECTION_LTR : LAYOUT_DIRECTION_RTL);
        mDialog.setCanceledOnTouchOutside(true);
        mDialog.setOnShowListener(dialog -> {
            mDialogView.getViewTreeObserver().addOnComputeInternalInsetsListener(this);
            if (!shouldSlideInVolumeTray()) {
                mDialogView.setTranslationX(
                        (mDialogView.getWidth() / 2.0f) * getTranslationForPanelLocation());
            }
            mDialogView.setAlpha(0);
            mDialogView.animate()
                    .alpha(1)
    ...
    private void updateRowsH(final VolumeRow activeRow, boolean animate) {
        ...
        boolean isOutmostIndexMax = mLeftVolumeRocker ? isRtl() : !isRtl();
    ...
    private void setTopContainerBackgroundDrawable() {
        ...
        background.setLayerGravity(
                0, mLeftVolumeRocker ? Gravity.TOP | Gravity.LEFT : Gravity.TOP | Gravity.RIGHT);
