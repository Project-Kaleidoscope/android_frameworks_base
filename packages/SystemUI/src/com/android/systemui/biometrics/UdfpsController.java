...
public class UdfpsController implements DozeReceiver {
    ...
    public class UdfpsOverlayController extends IUdfpsOverlayController.Stub {
        ...
        public void onAcquired(int sensorId, int acquiredInfo, int vendorCode) {
            mFgExecutor.execute(() -> {
                final boolean isAodEnabled =
                        mAmbientDisplayConfiguration.alwaysOnEnabled(UserHandle.USER_CURRENT);
                final boolean isShowingAmbientDisplay =
                        mStatusBarStateController.isDozing() && mScreenOn;
                if (acquiredInfo == 6 && ((mScreenOffFod && !mScreenOn) ||
                                          (isAodEnabled && isShowingAmbientDisplay))) {
    ...
    private void updateScreenOffFodState() {
        ...
        mScreenOffFod = isSupported &&
                mSecureSettings.getInt(Settings.Secure.SCREEN_OFF_UDFPS_ENABLED, 1) == 1;
    }
